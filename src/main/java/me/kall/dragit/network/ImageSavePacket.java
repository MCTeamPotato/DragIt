...
public class ImageSavePacket {
    ...
    public ImageSavePacket(ResourceLocation dimension, long pos, ResourceLocation textureLocation, byte[] textureBytes) {
    ...
    public void handle(@NotNull Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            savedImages.images.computeIfAbsent(this.dimension, key -> new Long2ObjectOpenHashMap<>()).put(this.pos, new SavedImages.Image(this.textureLocation, this.textureBytes));
            ...
            for (ServerPlayer syncTarget : syncTargets) {
                ...
                ImageSyncManager.INSTANCE.send(PacketDistributor.PLAYER.with(() -> syncTarget), new ImageLoadPacket(this.dimension, this.pos, this.textureLocation, this.textureBytes));
            }
        });
